...
class ConcurrentExamplesTest < ActiveSupport::TestCase
  ...
  setup do
    Sidekiq.configure_client do |cfg|
      ...
      cfg.redis = { size: 25 }
      cfg.queues = ['default'] if cfg.respond_to?(:queues) # sidekiq >= 7
    end
    Sidekiq::Testing.disable!

    SidekiqSmartCache.redis.flushdb
    config = if Sidekiq.respond_to?(:options)
      # Sidekiq < 7 compatibility
      Sidekiq.options.merge(queues: ['default'])
    else
  ...
  test 'model timeout and then succeed' do
    ...
    assert_includes 1.5..2.5, duration, "Should finish after about two seconds" # more leeway for errors adding up
  end
